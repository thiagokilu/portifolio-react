...
const services = [
  ...
  { category: "Suporte", title: "Manutenção Industrial", index: "04" },
];
      ...
      >
        ...
        <motion.header variants={fadeUp} className="mb-24 space-y-4">
          <div className="flex items-center gap-4 overflow-hidden">
             <motion.div variants={lineExpand} className="h-px w-12 bg-current/30 origin-left" />
             <span className="text-[10px] uppercase tracking-[0.5em] opacity-40 font-sans">
               Capacidades
             </span>
          </div>
          <h2 className="text-6xl md:text-8xl font-semibold tracking-tighter leading-[0.85]"
          ...
          {services.map((service) => (
            ...
            >
              <div className="flex flex-col gap-6">
                ...
                {/* Título com Serif */}
                <h3 className="text-3xl md:text-5xl font-semibold tracking-tight group-hover:italic transition-all duration-500 ease-in-out"
                    style={{ fontFamily: "var(--font-display)" }}>
                  {service.title}
                </h3>
              </div>
              ...
              <div className="mt-8 md:mt-0 flex items-center gap-3 opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all duration-700">
                 <span className="text-[10px] uppercase tracking-widest opacity-50">Detalhes</span>
